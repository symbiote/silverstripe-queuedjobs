---
title: Defining Jobs
summary: Creating custom queued jobs, covering the API, constructors, job state management, multiple steps, and advanced setup patterns.
icon: cogs
---
...
- [`PublishItemsJob`](api:Symbiote\QueuedJobs\Jobs\PublishItemsJob) - A job used to publish all the children of a particular node. To create this job, run the PublishItemsTask passing in the parent as a request var (eg ?parent=1)
- [`GenerateGoogleSitemapJob`](api:Symbiote\QueuedJobs\Jobs\GenerateGoogleSitemapJob) - A job used to create a google sitemap. If the googlesitemaps module is installed it will include priority settings as defined there, otherwise just produces a generic structure. To create an initial instance of this job, call `dev/tasks/CreateDummyJob?name=GenerateGoogleSitemapJob`. This will create the initial job and queue it; once the job has been run once, it will automatically schedule itself to be run again 24 hours later.

## API overview

The module comes with an [`AbstractQueuedJob`](api:Symbiote\QueuedJobs\Services\AbstractQueuedJob) class that defines many of the boilerplate functionality for the
job to execute within the framework. An example job can be found in [`PublishItemsJob`](api:Symbiote\QueuedJobs\Jobs\PublishItemsJob).

The key points to be aware of are

- When defining the constructor for your job, be aware that the [`QueuedJobService`](api:Symbiote\QueuedJobs\Services\QueuedJobService) will, when
...
- **Job Properties** QueuedJobs inherited from the `AbstractQueuedJob` have a default mechanism for persisting values via the __set and__get mechanism that stores items in the *jobData* map, which is serialized between executions of the job processing. All you need to do from within your job is call `$this->myProperty = 'somevalue';`, and it will be automatically persisted for you; HOWEVER, on subsequent creation of the job object (ie, in the `__constructor()`) these properties *have not* been loaded, so you *cannot* rely on them at that point.
...
- **Job Signatures** When a job is added to the job queue, it is assigned a unique key based on the parameters of the job
(see [`AbstractQueuedJob::getSignature()`](api:Symbiote\QueuedJobs\Services\AbstractQueuedJob::getSignature())). If a job is already in the queue with the same signature, the new job
...
- **Job Type** You can use either `QueuedJob::QUEUED`, which will mean the job will run within a minute (due to the cronjob), or `QueuedJob::IMMEDIATE`, which will execute the job as soon as possible. This forces execution of the job at the end of the currently
processing request, OR if you have set [`QueuedJobService.use_shutdown_function`](api:Symbiote\QueuedJobs\Services\QueuedJobService->use_shutdown_function) to `false`, a monitoring job to trigger the execution of the job queue (see the [lsyncd](./04_immediate-jobs.md#lsyncd) config section). This job type is useful for doing small things (such as deleting a few items at a time, indexing content in a separate search indexer, etc)
- **queueJob()** To actually add a job to a queue, you call [`QueuedJobService::queueJob()`](api:Symbiote\QueuedJobs\Services\QueuedJobService::queueJob()).
...
- **Switching Users** Jobs can be specified to run as a particular user. By default this is the user that created
...
A subclass to define your own queued jobs based upon. You don't need to use it, but it takes care of a lot of stuff for you.

### `QueuedJobService`

The a service for registering instances of queued jobs

### `ProcessJobQueueTask`

[`ProcessJobQueueTask`](api:Symbiote\QueuedJobs\Tasks\ProcessJobQueueTask) is the task you run to have queued jobs processed. This must be set up to run via cron.

### `QueuedJobDescriptor`

A [`QueuedJobDescriptor`](api:Symbiote\QueuedJobs\DataObjects\QueuedJobDescriptor) is the stored representation of a piece of work that could take a while to execute,
...
The currently executing job step can also be observed in the CMS via the *Jobs admin* UI. This is useful mostly for debugging purposes when monitoring job execution.
...
Don't forget that in your unit test you must call [`AbstractQueuedJob::process()`](api:Symbiote\QueuedJobs\Services\AbstractQueuedJob::process()) as many times as you have items in your test data as one `process()` call handles only one item.
...
1. job is queued, the matching job descriptor is saved into the database
1. job is picked and processed up by the queue runner.
...
##### Job class constructor {#opt1-job-class-contructor}

```php
// no constructor
```

##### Job creation {#opt1-job-creation}
...
##### Job class constructor {#opt2-job-class-contructor}
...
##### Job creation {#opt2-job-creation}

```php
$job = new MyJob($string, $integer, $array);
```

##### Advantages {#opt2-advantages}

- typed parameters.

##### Disadvantages {#opt2-disadvantages}
...
This approach is especially problematic on PHP 7.3 or higher as the static syntax checker may have an issue with nullable values and force you to implement additional check like [`is_countable()`](https://www.php.net/manual/en/function.is-countable.php) on the job properties.
...
##### Job class constructor {#opt3-job-class-contructor}

```php
public function __construct(array $params = [])
{
    if (
        !array_key_exists('string', $params)
        || !array_key_exists('integer', $params)
        || !array_key_exists('array', $params)
    ) {
        // job constructor called by the queue runner - exit early
        return;
    }
...
##### Job creation {#opt3-job-creation}

```php
$job = new MyJob(['string' => $string, 'integer' => $integer, 'array' => $array]);
```

##### Advantages {#opt3-advantages}

- nullable values don't need to be handled.

##### Disadvantages {#opt3-disadvantages}
...
This approach is the strictest when it comes to validating parameters but requires the [`Job::hydrate()`](api:SilverStripe\StaticPublishQueue\Job::hydrate()) method to be called after each job creation.
Don't forget to call `hydrate()` in your unit test as well.
...
##### Job class constructor {#opt4-job-class-contructor}
...
##### Job creation {#opt4-job-creation}
